...
writer = pd.ExcelWriter(attendance_file, engine="openpyxl", mode="a", if_sheet_exists="overlay")
...
if current_date not in writer.sheets:
    ...
    attendance_df.to_excel(writer, sheet_name=current_date, startrow=start_row, header=False, index=False)
